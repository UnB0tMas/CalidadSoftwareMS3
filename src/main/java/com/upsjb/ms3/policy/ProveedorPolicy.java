...
public class ProveedorPolicy {
    ...
    public void ensureCanCreate(AuthenticatedUserContext actor, boolean employeeCanRegisterEntry) {
        PolicyGuard.ensureCan(canCreate(actor, employeeCanRegisterEntry), "PROVEEDOR_CREAR_DENEGADO", "registrar proveedor");
    }

    public void ensureCanUpdate(AuthenticatedUserContext actor, boolean employeeCanRegisterEntry) {
        PolicyGuard.ensureCan(canUpdate(actor, employeeCanRegisterEntry), "PROVEEDOR_EDITAR_DENEGADO", "editar proveedor");
    }

    public void ensureCanChangeState(AuthenticatedUserContext actor) {
        PolicyGuard.ensureCan(canChangeState(actor), "PROVEEDOR_ESTADO_DENEGADO", "cambiar estado de proveedor");
    }

    public void ensureCanViewAdmin(AuthenticatedUserContext actor) {
        PolicyGuard.ensureCan(canViewAdmin(actor), "PROVEEDOR_CONSULTA_DENEGADA", "consultar proveedores");
    }
}