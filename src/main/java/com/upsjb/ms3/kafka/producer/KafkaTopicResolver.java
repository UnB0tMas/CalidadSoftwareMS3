...
public class KafkaTopicResolver {
    ...
    public String resolveMs4StockCommandTopic() {
        return properties
                .resolveMs4StockCommandTopic();
    }

    public String resolveMs4StockReconciliationTopic() {
        return properties
                .resolveMs4StockReconciliationTopic();
    }
    ...
    private String resolveByEventType(
            String eventType
    ) {
        if (!StringUtils.hasText(
            ...
            throw new IllegalArgumentException(
                    "No se pudo resolver topic Kafka: eventType vacío."
            );
        }
        ...
        if (
                value.contains("promocion")
                        || value.contains("promoción")
        ) {
            return properties
                    .resolvePromocionSnapshotTopic();
        }
        ...
        if (
                value.contains("movimiento")
                        || value.contains("inventario")
        ) {
            return properties
                    .resolveMovimientoInventarioTopic();
        }
        ...
        throw new IllegalArgumentException(
                "No existe topic Kafka configurado para eventType: "
                        + eventType
        );
    }
}